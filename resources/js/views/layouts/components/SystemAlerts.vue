<template>
  <div class="banner--container">
    ...
    <div v-if="!socketConnection" class="banner banner-danger">
      We are unable to connect you with CodePier's servers, you may not receive
      updates properly ....
    </div>
    <div v-if="current_version !== version" class="banner banner-info">
      <div>
        Hello, we've got a new version of CodePier ready for you! Please
        <a href="" class="btn btn-small">refresh now</a>
      </div>
    </div>
    <div class="banner banner-info" v-if="!user.confirmed">
      You have not confirmed your email. You will be unable to create servers
      until confirmed.
      <a class="btn btn-small" @click="resendConfirmation()">Resend Now</a>
    </div>
  </div>
</template>
