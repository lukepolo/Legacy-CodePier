<template>
  <div class="grid--item">
    <div class="flex flex--center heading">
      <h3 class="flex--grow text-center">Lifelines</h3>

      <tooltip message="Add Lifeline">
        <span
          class="btn btn-small btn-primary"
          :class="{ 'btn-disabled': !this.showLifelineForm }"
          @click="showLifelineForm = !showLifelineForm"
        >
    ...
    <div v-if="!lifeLines.length">
      <p>
        Lifelines enable you to add monitoring to your services by pinging a URL
        periodically. If the time goes over your set threshold, CodePier will
        let you know.
      </p>
      <div class="text-empty text-center">
        You don't have any lifelines active.
      </div>
    </div>
    <div v-else>
      <p>
        Add the provided URL (<span class="icon-clipboard"></span>) to your
        service to start monitoring.<br />
      </p>
    </div>
      ...
      <form @submit.prevent="createLifeline">
        ...
        <div class="flyform--group">
          ...
          <div class="flyform--group-postfix">
            ...
            <label></label>
            <template>
              <div class="flyform--group-postfix-label">minutes</div>
            </template>
          </div>
        </div>
