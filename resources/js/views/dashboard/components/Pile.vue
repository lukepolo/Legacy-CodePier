      ...
      <h4>
        <template v-if="editing">
          ...
          <div class="action-btn">
            <button @click="savePile" class="btn btn-small btn-primary">
        ...
        <template v-else>
          ...
          <div class="action-btn">
            <button @click="editing = true" class="btn btn-small">
    ...
    <div class="group--item-content" v-if="pile">
      <template v-if="sites && sites.length">
        ...
        <div class="list">
          <router-link
            class="list--item"
            :to="{ name: 'site', params: { site: site.id } }"
            v-for="site in sites"
            :key="site.id"
          >
            <div class="list--item-name">{{ site.name }}</div>
          </router-link>
        </div>
      </template>
      <template v-else>
      ...
      <div
        :class="{ disabled: !siteCreateEnabled }"
        class="group--item-link"
        @click="addingSite = true"
