...
export const update = (context, data) => {
    return Vue.request(data.form).post(
...
export const installOnServer = (context, data) => {
    return Vue.request(data).post(
        Vue.action('Server\ServerBuoyController@store', { server: data.server })
    ).then(() => {
        app.$router.push({ name : 'server_buoys', params : { server_id : data.server } })
    })
}