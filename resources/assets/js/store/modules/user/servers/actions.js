...
export const archive = (context, server) => {
    return Vue.request(server)
        .delete(
            Vue.action("ServerServerController@destroy", { server: server }),
            ["user_servers/remove", "user_site_servers/remove"]
        )
        .then(() => {
...
export const restore = ({ dispatch }, server) => {
    return Vue.request(server)
        .post(
            Vue.action("ServerServerController@restore", { server: server }),
            ["user_servers/add", "user_servers/removeFromTrash"]
        )
        .then(server => {
            dispatch("listenTo", server);
            return server;
