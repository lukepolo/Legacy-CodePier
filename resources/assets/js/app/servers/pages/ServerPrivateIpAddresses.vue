<template>
  <div>
    <div class="flex flex--center">
      <h3 class="flex--grow">Private IP Addresses</h3>
    ...
    <p>
      CodePier uses Private ip addresses to help facilitate load balancers,
      firewall rules from Database servers , and worker servers.
    </p>

    <div class="alert alert-info">
      If a private IP address is changed, the server will automatically run
      <code><strong>Fix Server Configuration</strong></code> from the help menu.
    </div>
          ...
          <td class="table--action">
            <tooltip message="Delete">
              <span class="table--action-delete">
                <a @click="deleteIp(privateIpAddress)"
    ...
    <form @submit.prevent="addIpAddress" v-if="shouldShowForm">
      ...
      <div class="flyform--footer">
        <div class="flyform--footer-btns">
          <span
            class="btn"
            v-if="privateIpAddresses.length"
...
export default {
  ...
  methods: {
    deleteIp(ip) {
      this.updateIpAddresses(
        this.privateIpAddresses.filter((privateIp) => {
          return privateIp !== ip;
        }),
      );
    },
  ...
  computed: {
    ...
    shouldShowForm() {
      return this.privateIpAddresses.length === 0 || this.showForm;
    },
    privateIpAddresses() {
