  ...
  <section>
    <template v-if="userSubscription">
      <div class="alert alert-warning" v-if="userSubscriptionData.isOnTrail">
        ...
        <strong>{{
          parseDate(userSubscription.trial_ends_at).format("l")
        }}</strong>
      </div>
      <div
        class="alert alert-error"
        v-if="
          userSubscription.ends_at && !userSubscriptionData.subscriptionEnded
        "
      >
        Your subscription has been canceled and will end on
        {{ parseDate(userSubscription.ends_at).format("l") }}
      </div>
    </template>
    ...
    <div class="pricing pricing-inapp">
      <div
        ...
        :class="{
          selected:
            !this.userSubscription || userSubscriptionData.subscriptionEnded,
        }"
      >
    ...
    <form @submit.prevent="subscribe" method="post">
      ...
      <div class="flyform--footer">
        ...
        <div class="flyform--footer-links" v-if="userSubscription">
          <button
            class="text-error"
            @click.prevent.stop="cancelSubscription"
            v-if="!isCanceled"
          >
            Cancel Subscription
          </button>
    ...
    <template v-if="invoices.length">
      ...
      <table>
        <tr v-for="invoice in invoices">
          ...
          <td class="text-right">
            <a :href="downloadLink(invoice.id)">Download</a>
          </td>
        </tr>
      </table>
    </template>
