<template>
  <div v-if="site">
    <h3 class="section-header">
      ...
      <div
        class="section-header--btn-right"
        v-if="
          workFlowCompleted === true &&
            ((siteServers && siteServers.length) || availableServers.length)
        "
      >
        <drop-down
          icon="fa fa-plus"
              ...
              <ul>
                <li>
                  <a href="#" @click.prevent="connectServers = !connectServers">
                    <span class="icon-server"></span> Add Already Provisioned
                    Server
                  </a>
                </li>
              </ul>
    ...
    <div class="section-content">
        ...
        <template v-if="!isSubscribed">
          <div class="slack-invite">
            <router-link :to="{ name: 'subscription' }">
              Upgrade Account
              <div class="small">
                The free plan only allows for 1 server. <br />Upgrade now to add
                more!
              </div>
            </router-link>
          </div>
        ...
        <template v-else-if="!serverCreateEnabled">
          <div class="slack-invite">
            <router-link :to="{ name: 'subscription' }">
              Upgrade Account
              <div class="small">
                Your current plan only allows for 30 servers. <br />Upgrade now
                to add more!
              </div>
            </router-link>
          </div>
      ...
      <template v-else>
        <template v-if="site.repository && workFlowCompleted === true">
              ...
              <div class="jcf-form-wrap">
                  ...
                  <form class="floating-labels">
                    <div class="jcf-input-group input-checkbox">
                      <label>
                        ...
                        />
                        <span class="icon"></span> {{ server.name }} ({{
                          server.ip
                        }})
                      </label>
                    </div>
                ...
                <div class="btn-footer">
                  <template v-if="siteServers">
                    <span
                      class="btn danger"
                      @click.prevent="resetAttachedServers"
                      >Cancel</span
                    >
                  </template>
          ...
          <template v-else>
            <h3 class="section-header--secondary">
              Lets create your first Server
            </h3>
          ...
          <div class="jcf-form-wrap">
            <div class="jcf-input-group">
              <h5 class="section-header--secondary">
                Please fill out your <br />app requirements before creating a
                server
              </h5>
            </div>
          </div>
