<template>
  <div class="parent">
    ...
    <section
      ...
      :class="{ 'in-workflow': workFlowCompleted !== true }"
    >
          ...
          <template v-if="site">
            ...
            <transition :name="transitionName">
              <template v-if="notOverview">
                ...
                >
                  <template
                    v-if="workFlowCompleted !== true && totalWorkflowSteps > 0"
                  >
                    <div class="flex">
                      <div class="flex--grow">
                        <h2>{{ workFlowName }}</h2>
                        <h4 class="secondary">
                          Workflow Step #{{ workflowStepsCompleted }} /
                          {{ totalWorkflowSteps }}
                        </h4>
                      </div>
                      ...
                      <template
                        v-if="
                          workFlowCompleted !== true && totalWorkflowSteps > 0
                        "
                      >
                        ...
                        <div class="flyform--footer-btns">
                          <button
                            @click="revertWorkFlow"
                            class="btn btn-small"
                            :disabled="workflowStepsCompleted === 1"
                            :class="{ disabled: workflowStepsCompleted === 1 }"
                          >
                            <span class="icon-arrow-left"></span> Previous Step
                          </button>
                          <button
                            @click="updateWorkFlow"
                            class="btn btn-small btn-primary"
                          >
                            Next Step <span class="icon-arrow-right"></span>
                          </button>
                        </div>
                      </template>
                    </div>
                    <div class="alert-info" v-if="workflowMessage">
                      <span class="alert-wrap" v-html="workflowMessage"></span>
                    </div>
                    <hr />
                  </template>
    ...
    <section id="right" v-if="site" class="section-column">
      <portal-target name="right-panel"> <servers></servers> </portal-target>
    </section>
  </div>
</template>
...
export default {
  ...
  watch: {
    ...
    site() {
      if (this.site && this.pileId !== this.site.pile_id) {
        this.$store
          .dispatch("user_piles/change", this.site.pile_id)
          .then(() => {
            app.$router.push({
              name: "site_overview",
              params: {
  ...
  computed: {
    ...
    workflowStepsCompleted() {
      return (
        _.filter(this.site.workflow, function(flow) {
