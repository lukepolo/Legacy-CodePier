<template>
  <div v-if="site">
    <form @submit.prevent="updateSite">
      ...
      <h3
        class="heading"
        v-if="!form.user_repository_provider_id && !form.custom_provider"
      >
      ...
      <template v-if="form.user_repository_provider_id || form.custom_provider">
        <div class="flyform--group">
          <div class="flyform--group-prefix">
            ...
            <template v-if="!form.custom_provider">
              <div class="flyform--group-prefix-label">{{ providerUrl }}/</div>
            </template>
          </div>
        ...
        <div class="flyform--group">
          <div class="flyform--group-prefix">
            <input
              ...
              placeholder=" "
            />
            <label for="web_directory" class="flyform--group-iconlabel"
              >Web Directory</label
            >
        ...
        <div class="flyform--group">
          ...
          <div
            class="flyform--group-select"
            v-if="
              Object.keys(availableLanguages).length &&
                Object.keys(availableFrameworks).length
            "
          >
            <select v-model="form.type" name="type" required>
              ...
              <template v-for="(features, language) in availableLanguages">
                <optgroup :label="language">
                  ...
                  <option
                    v-for="(features, framework) in availableFrameworks[
                      language
                    ]"
                    :value="language + '.' + framework"
                  >
                    {{ framework | startCase }}</option
                  >
                </optgroup>
              </template>
            </select>
