...
(function($) {
  ...
  for (var i = 0; i < periods.length; i++) {
    str_opt_period +=
      "<option value='" + periods[i] + "'>" + periods[i] + '</option>\n';
  }
  ...
  function getCronType(cron_str) {
    ...
    for (var i = 0; i < d.length; i++) {
      ...
      $.error(
        'cron: invalid value found (col ' + (i + 1) + ') in ' + o.initial,
      );
      return undefined;
    }
  ...
  function hasError(c, o) {
    if (!defined(getCronType(o.initial))) {
      return true;
    }
    ...
    init: function(opts) {
      ...
      block['time'] = $(
        "<span class='cron-block cron-block-time'>" +
          " at <select name='cron-time-hour' class='cron-time-hour'>" +
          str_opt_hid +
